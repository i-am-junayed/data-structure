### QUESTION : Write findMaxR(self, root) that will return the minimum of a BST using recursion. 
...
class BinarySearchTree:
  ...
  def find_mix_value(self):
    current = self.root
    while current.left_child:
        current = current.left_child
    return current.data
    
  # Find the node with the minimum value using recursion. 
  # So basically only this function is the answer of this question. 
  
  def find_min_using_recursion(self, root):
    if root == None:
      return
...
bst.find_min_using_recursion(bst.root)
